...
- **Hybrid runtime.** A deterministic orchestration harness that runs the real Odoo build with **no API
...
- **Odoo-build half only.** Build the ERP (connect → install → configure → master data → products/rooms →
  POS/booking → transactions → invoice/pay → automations → QA). **Do not** build the website/strategy-site
  generation half.
- **Language:** Python 3.11+, standard library + the official `anthropic` SDK only. No other
  third-party runtime deps. (`pytest` for tests is fine.)
- **Model:** `claude-opus-4-8` (override via env `BLACKBOX_MODEL`). Use adaptive thinking
...
- **Secrets** come from environment / MCP config and are **NEVER committed**. Ship a `.env.example` with
...
- `odoo-build-spec/connect/probe.py` — **REUSE.** Tier selection: `probe_ssh()` / `probe_api()` /
  `probe_browser()` → recommended tier (ssh → api → browser).
...
```
state = load_state()                          # resume from where we left off
ctx   = build_context(config, spec, client)   # spec + Odoo facade + observability
for stage in STAGES:                           # ordered 0..9
    if state[stage] == "done":
        continue
    for attempt in range(1, MAX_ATTEMPTS + 1): # L2 per-stage bounded retry (MAX_ATTEMPTS = 3)
        approvals.gate(stage, ctx)             # L6 human-in-the-loop (destructive/pricing/comms)
        AGENTS[stage].run(ctx)                 # idempotent: search-before-create
        result = verification.verify(stage, ctx)
        if result.passed:
            state[stage] = "done"; save_state(state); break
        if ctx.has_api_key:                    # L3 ReAct reasoning layer (only when keyed)
            llm.react_fix(stage, result.gaps, ctx)
        observability.log_gaps(stage, result.gaps)
    else:                                      # exhausted attempts → escalate, never thrash
        escalate(stage, result.gaps); break
report.emit(ctx)                               # counts vs targets, deviations
```
...
**COUNT_KEYS to track:** Modules, Customers, Employees, Products, Rooms, Sale orders, POS orders, Invoices,
Automations, CRM. **Targets (from `demo-data.json`):** customers 150, employees ~36, products ~42,
rooms 24, `sale.order` ≥ 600, `pos.order` ~2000, invoices ~400, automations ~27, crm ≥ 80.
...
- `python -m blackbox --help` and all subcommands work.
- `python -m blackbox run --dry-run` executes all 9 stages against `FakeOdoo`, prints the
  loop/gate/verification trace, and reports counts within ±10% of targets — no network, no API key.
- `python -m blackbox probe` degrades gracefully with no env set.
- With `ODOO_*` set, run connects via `xmlrpc_client` and idempotently executes the data stages;
  re-running does not duplicate records.
- With `ANTHROPIC_API_KEY` set, the LLM runtime initializes and the ReAct loop is exercised (e.g. for
  gap-fixing); without it, the engine runs fully deterministically.
- No secrets in the repo; `.env.example` has placeholders only; `.gitignore` covers `.env`.
- `pytest` passes (spec load, idempotency no-dup, verification logic, dry-run end-to-end).
- `blackbox/README.md` documents env vars, tiers, `--dry-run`, and the SaaS-only manual prerequisites.
